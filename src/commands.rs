...
pub fn suggest_command_local(mistyped: &str, commands: &[String]) -> Option<String> {
    ...
    for cmd in commands {
        // O(1) Short-circuit: Si la commande est beaucoup plus longue ou plus courte, on ignore.
        // On fait ça avant le calcul O(N*M) de levenshtein pour des gains de performance.
