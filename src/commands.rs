        ...
        for path in env::split_paths(&path_var) {
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() {
                    // Bolt ⚡: Optimize fs iteration by using entry.file_type() instead of entry.path().is_file()
                    // This avoids redundant `stat` system calls and unnecessary `PathBuf` allocations
