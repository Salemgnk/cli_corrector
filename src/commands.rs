            ...
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() {
                    let is_file = match entry.file_type() {
                        // Optimisation: utiliser le file_type du dirent pour éviter
                        // un appel stat redondant (sauf si c'est un lien symbolique)
                    ...
                    if is_file {
                        // Utiliser entry.file_name() au lieu de entry.path().file_name()
                        // pour éviter une allocation PathBuf
