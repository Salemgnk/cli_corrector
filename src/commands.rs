        ...
        for path in env::split_paths(&path_var) {
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() {
                    // ⚡ Bolt Optimization: Using `entry.file_type()` and `entry.file_name()`
                    // avoids redundant 'stat' system calls and 'PathBuf' allocations.
                    // IMPORTANT: We must still handle symlinks by falling back to `entry.path().is_file()`,
                    // since `file_type()` does not follow symlinks, and many commands in $PATH are symlinks.
