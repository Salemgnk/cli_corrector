        ...
        for path in env::split_paths(&path_var) {
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() {
                    // Bolt ⚡ Optimization:
                    // Using entry.file_type() avoids redundant `stat` system calls that occur
                    // when using entry.path().is_file(), which drastically improves PATH traversal speed.
                    // Fallback to entry.path().is_file() is only necessary for symlinks.
                    ...
                    if is_file {
                        // Bolt ⚡ Optimization:
                        // Using entry.file_name() directly prevents unnecessary PathBuf allocations.
