        ...
        for path in env::split_paths(&path_var) {
            if let Ok(entries) = fs::read_dir(path) {
                for entry in entries.flatten() {
                    // Performance optimization: use `entry.file_type()` to avoid redundant `stat` syscalls,
                    // but fallback to `entry.path().is_file()` for symlinks (common in $PATH).
                    ...
                    if is_file {
                        ...
                        // to avoid unnecessary PathBuf heap allocation.
